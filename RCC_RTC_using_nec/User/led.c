...
void MCO_PA8_GPIO_Init(void)   //配置MCO 端口 PA8 模式配置  
{
		...
		GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AF_PP;   //选择推挽输出
...
void LEDout_P15_GPIO_Init(void) //配置PA15  端口 PA8 模式配置 led点亮输出
{
		...
		GPIO_InitStruct.GPIO_Pin =  GPIO_Pin_15;                     //选择第 0 位IO 	
...
//-----------------------------------------------------------		
	...
	Delay_us(30000);  //延迟   不是毫秒只是起到延迟的效果		
