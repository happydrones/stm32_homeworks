...
#define BUF_LEN 32
volatile char    rx_buf[BUF_LEN];   // 接收缓冲区
volatile uint8_t rx_index = 0;        // 当前索引
volatile uint8_t rx_flag  = 0;         // 一帧数据接收完成标志
uint8_t 		 get_string_flag=0 ;
uint8_t single_char;
...
void USART1_IRQHandler(void)
{
	...
	{  //接收中断(接收到的数据必须是0x0d 0x0a结尾)		
		single_char =USART_ReceiveData(USART1);						//读取接收到的数据
		get_string_flag =1;
		//接收连续字符串
		if (single_char == '\r')
		{
        ...
        {
            if(rx_index < BUF_LEN - 1)  // 防止越界
            {    
				rx_buf[rx_index] = single_char;
				rx_index =rx_index + 1;
			}
        }  									//把收到的数据发送回电脑
