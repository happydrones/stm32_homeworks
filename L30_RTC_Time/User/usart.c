...
#define BUF_LEN 32
volatile uint8_t Rx_BufferIndex = 0;        // 接收缓冲区索引
...
/**
  ...
  *         - Handles \r\n terminated strings (CR+LF format)
  ...
  * 3. Processes CR(\r)/LF(\n) sequence:
  *    - CR marks beginning of line ending
  *    - LF completes message when following CR
  * 4. Stores valid characters in rx_buf with index management
  * 5. Sets rx_flag when message complete
  * @endverbatim
  */
void USART1_IRQHandler(void)
{
  	static uint8_t saw_CR =0;

	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
	{  //接收中断(接收到的数据必须是0x0d 0x0a结尾)		
		rx_data_buffer =USART_ReceiveData(USART1);						//读取接收到的数据
		...
		else if (rx_data_buffer == '\n' && saw_CR) 
		{
			USART1_RX_BUF[Rx_BufferIndex] = '\0';
			Rx_Data_Flag =1;
			Rx_BufferIndex = 0;
			saw_CR = 0;
		}
        ...
        {
            if(Rx_BufferIndex < BUF_LEN - 1)  // 防止越界
            {    
				USART1_RX_BUF[Rx_BufferIndex] = rx_data_buffer;
				Rx_BufferIndex =Rx_BufferIndex + 1;
			}
        }  									//把收到的数据发送回电脑
