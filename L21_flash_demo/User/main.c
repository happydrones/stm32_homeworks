/**
  ...
  * @version V1.4.0 //增加了flash保存数码管亮度掉电保持 
  * @attention
  * 注意事项：
...
#define Flash_address 0x0807F800
...
//	Single_Led_Init(LED_PORT, down_LED);
	...
	while(1)
	{
/*该部分是关于实现四个方向键按动下去，对应的led持续闪烁，然后松手仍然保持闪烁
		直到下一个按键摁下。
...
//setting_mode =0时候，为正常的时间工作状态
		if(setting_mode == 0)
		{
			...
			{	
				Seconds++;
				Time_Write(Hours, Minutes,  Seconds);
...
//首先进入setting mode=1 ,然后按一次方向键，进入设置秒模式，将setting mode置为2
