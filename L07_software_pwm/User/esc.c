...
{
//	GPIO_WriteBit(GPIOA, GPIO_Pin, Bit_SET);  // 先设高电平
//	delay_ms(100);                               // 延时观察效果
//	GPIO_WriteBit(GPIOA, GPIO_Pin, Bit_RESET); // 再设低电平
//	delay_ms(100);
    ...
    uint8_t Pwm_esc_frequency = 50;
    uint8_t loop_times = 10;

  
    uint16_t high_time=0, low_time=0;
    uint16_t T =100000/Pwm_esc_frequency; //周期大小，单位为10us
    high_time = T * 5/100 + T * speed/2000; //高电平时间(ms),占空比为5%的基础上进行
